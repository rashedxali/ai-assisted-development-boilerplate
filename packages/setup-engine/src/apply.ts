...
async function mergeEnvExample(
  ...
  if (selections.infisical) {
    const infisicalEnv = await readTextFile(
      join(repoRoot, "addons/infisical/.env.example"),
    );
    if (infisicalEnv) sections.push(infisicalEnv.trim());
  }
...
const INFISICAL_SCRIPT_KEYS = ["dev:build", "dev:start"] as const;

function patchInfisicalScripts(
  pkg: PackageJson,
  selections: SetupOptions["selections"],
): void {
  if (!pkg.scripts) pkg.scripts = {};
