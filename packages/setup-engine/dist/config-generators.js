export function generateNextConfig(selections) {
    const hasFumadocs = selections.fumadocs;
    const hasSentry = selections.sentry;
    const lines = ['import type { NextConfig } from "next";'];
    if (hasFumadocs) {
...
export function generateTsConfig(selections) {
    const base = {
        compilerOptions: {
            target: "ES2017",
...
export function generateEslintConfig(selections) {
    const ignores = [
        ".next/**",
        "out/**",
        "build/**",
...
export function generateEnvConfig(selections) {
